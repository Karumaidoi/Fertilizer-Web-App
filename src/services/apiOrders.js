...
export async function getBooks() {
  const { data, error } = await supabase.from('Fertilizer').select().order('id', { ascending: false });
...
export async function deleteOrder(id) {
  const { error } = await supabase.from('Fertilizer').delete().eq('id', id);
...
export async function createBook(newProject) {
  ...
  const { data, error } = await supabase.from('Fertilizer').insert([{ ...newProject, image: projectPath }]);
...
export async function updateOrder(newOrder) {
  ...
  const { data, error } = await supabase.from('Fertilizer').update(orderUpdated).eq('id', orderId);
